...
THEMAINLOGFILEB=$RECENTRUNLOGSDIRRAWMD
else
THEMAINLOGFILEA="Error Setting Run Log Link"
THEMAINLOGFILEB="Error Setting Run Log Link"
fi
if
[[ -f $MAINREADME ]]
then
sed -i "s/RECENTRUNMAINLINKA/$THEMAINLOGFILEA/" $MAINREADME
sed -i "s/RECENTRUNMAINLINKB/$THEMAINLOGFILEB/" $MAINREADME
fi
...
LISTSWITHOUTHTTPSLOGB=$RECENTRUNWITHOUTHTTPSMD
else
LISTSWITHOUTHTTPSLOGA="Error Setting https-less Link"
LISTSWITHOUTHTTPSLOGB="Error Setting https-less Link"
fi
if
[[ -f $MAINREADME ]]
then
sed -i "s/LISTSWITHOUTHTTPSLINKA/$LISTSWITHOUTHTTPSLOGA/" $MAINREADME
sed -i "s/LISTSWITHOUTHTTPSLINKB/$LISTSWITHOUTHTTPSLOGB/" $MAINREADME
fi
