...
gawk 'NR==FNR{a[$0];next} !($0 in a)' $COMBINEDWHITELISTS $COMBINEDBLACKLISTS >> $COMBINEDBLACKLISTSDBB
#grep -Fvxf $COMBINEDWHITELISTS $COMBINEDBLACKLISTS >> $COMBINEDBLACKLISTSDBB
#comm -23 $COMBINEDBLACKLISTS $COMBINEDWHITELISTS > $COMBINEDBLACKLISTSDBB
echo -e "`wc -l $COMBINEDBLACKLISTSDBB | cut -d " " -f 1` lines after whitelist"
echo ""
fi
...
EDITEDALLPARSEDSIZEBYTES=$(stat -c%s "$COMBINEDBLACKLISTSDBB")
...
then
printf "$yellow"  "Size of DBB List = $EDITEDALLPARSEDSIZEMB MB."
elif
[[ "$EDITEDALLPARSEDSIZEMB" -eq 0 && "$EDITEDALLPARSEDSIZEKB" -gt 0 && "$EDITEDALLPARSEDSIZEBYTES" -gt 0 ]]
then
printf "$yellow"  "Size of DBB ListE = $EDITEDALLPARSEDSIZEKB KB."
elif
[[ "$EDITEDALLPARSEDSIZEMB" -eq 0 && "$EDITEDALLPARSEDSIZEKB" -eq 0 && "$EDITEDALLPARSEDSIZEBYTES" -gt 0 ]]
then
printf "$yellow"  "Size of DBB List = $EDITEDALLPARSEDSIZEBYTES Bytes."
fi

if
[[ "$EDITEDALLPARSEDSIZEBYTES" -gt 0 ]]
then
EDITEDALLPARSEDHOWMANYLINES=$(echo -e "`wc -l $COMBINEDBLACKLISTSDBB | cut -d " " -f 1`")
...
[[ "$EDITEDALLPARSEDSIZEMB" -lt "$GITHUBLIMITMB" && -f $COMBINEDBLACKLISTSDBB ]]
then
